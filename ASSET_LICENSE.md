...
根目录 `LICENSE` 中的 MIT License 适用于本项目自行编写的程序代码、测试代码、构建脚本和项目配置。

## 不属于 MIT 的内容

以下内容由项目所有者保留全部权利，不包含在 MIT License 的授权范围内：
...
未经角色素材权利人明确许可，不得单独提取、再分发、二次打包、训练模型或用于商业用途。

你可以依据 MIT License 使用和修改程序代码，但需要自行替换上述角色素材，除非已经获得额外授权。

第三方项目和依赖仍分别遵循其原有许可证，详见 `THIRD_PARTY_NOTICES.md`。
...
them commercially. You may use and modify the code under the MIT License, but
you must replace the reserved character assets unless separately authorized.
