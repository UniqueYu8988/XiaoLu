...
const input = resolve(process.argv[2] ?? join(root, "voice-source", "xiaolu-supervision-voice-master.mp3"));
const output = join(root, "assets", "voice");
const temporary = join(root, "tmp", "supervision-voice-master-normalized.wav");

const clipNames = [
  ...Array.from({ length: 8 }, (_, index) => `patrol-start-playful-${index + 1}`),
  ...Array.from({ length: 6 }, (_, index) => `patrol-start-firm-${index + 1}`),
  ...Array.from({ length: 8 }, (_, index) => `patrol-return-${index + 1}`),
  ...Array.from({ length: 3 }, (_, index) => `patrol-success-${index + 1}`),
  ...Array.from({ length: 8 }, (_, index) => `night-stroll-${index + 1}`),
  ...Array.from({ length: 3 }, (_, index) => `night-resume-${index + 1}`),
];
...
console.log(`Created ${clipNames.length} supervision clips in ${output} from ${basename(input)} without removing existing voices.`);
