<div align="center">
  ...
  <p>
    <img alt="Version" src="https://img.shields.io/badge/version-1.8.3-76558f" />
...
说明书介绍了打卡、计时、自动目标、任务、书签、四档强监督巡逻、离线语音、YuQuiz 联动，以及小鹿的位置与提醒机制；其中“今日、任务、记录、统计、书签”均使用 1.6.0 的真实界面和虚构演示数据。可以直接[下载 PDF](docs/xiaolu-study-guide.pdf)，也可以在下方展开阅读。
...
核心学习数据集中在 `src/game.ts`，包括计时、打卡、自动目标、任务、书签和统计；窗口、动作、通知、语音、YuQuiz 联动与自动移动主要位于 `src/main.ts`。本地状态保存为 JSON，不需要服务器或账号。
...
强监督时段内，打开日记菜单不再暂停小鹿的巡逻：菜单会自动收起，小鹿继续提醒。超过 15 分钟仍未开始或回到有效学习时，应用会把 YuQuiz 学习台带到前台；页面已打开时复用并选中原标签页，只有确实未打开时才创建页面。超时后若再次切去其他标签页，小鹿会持续把学习台拉回，同时保持在网页上方显示；进入有效学习或 AI 查询宽限后解除约束。

### 5. 用本机 API 联动其他学习工具

YuQuiz 在 `127.0.0.1:8765` 暴露少量 companion 状态，共学日记只读取“页面是否打开、是否处于有效学习、今日题量、正确率与笔记统计”等摘要。题目、答案、笔记正文和密钥都不需要共享。
