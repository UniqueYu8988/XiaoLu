<div align="center">
  <img src="assets/icons/app-icon.svg" width="112" alt="小鹿图标" />
  <h1>共学日记</h1>
  <p>不是桌面宠物，而是住在桌面上的学习搭子。</p>
  <p>
    <img alt="Version" src="https://img.shields.io/badge/version-1.1.3-76558f" />
    <img alt="Platform" src="https://img.shields.io/badge/platform-Windows-4b315e" />
    <img alt="Data" src="https://img.shields.io/badge/data-local--only-a9c99e" />
    <img alt="License" src="https://img.shields.io/badge/code%20license-MIT-f4d57b" />
  </p>
</div>

## 她是谁

小鹿是一位本地运行的桌面学习搭子。她来自一份 Codex 动画宠物素材，但这个项目把她从 Codex 中独立出来，重新做成了一个专注于现实学习约定的 Windows 应用。

她不会评价学习质量，也不会要求连续学习十二个小时。她只做几件简单的事：

- 在固定时间问一句“你在学习现场吗”；
- 在上午、下午和晚上各陪你跨过一次“迟迟不开始”；
- 由你主动开始和结束每一段学习计时；
- 记下当天任务，并让固定任务在每天自动刷新；
- 在晚上收好当天的学习成果和现实约定；
- 用两份每日悬赏赢得各自的书签；两个人都完成约定时，再留下一枚双人书签。
- 可选读取本机 YuQuiz 的学习摘要，自动同步网页学习时间和今日答题量。

> 小鹿不是 OpenAI 或 Codex 官方产品。安装后的应用可以完全脱离 Codex 独立运行。

## 安装

1. 打开 [Releases](https://github.com/UniqueYu8988/XiaoLu/releases)，下载最新的 `xiaolu-study-mate-*-x64.msi`。
2. 双击 MSI，按安装向导完成安装。
3. 首次启动后，小鹿会出现在桌面并加入 Windows 登录自启动。

当前安装包没有商业代码签名，Windows 可能显示“未知发布者”。项目不会联网，也不会上传学习数据。

## 最常用的三个操作

| 操作 | 作用 |
| --- | --- |
| 双击小鹿 | 发起一次学习启动；学习中再次双击可结束手动计时 |
| 右键小鹿 | 打开像素风“共学日记” |
| 拖动小鹿 | 调整桌面位置，拖动期间播放跑步动作 |

暂停时不需要专门登记。没有计时的小鹿会恢复平常状态，并继续看向鼠标；下一次想学习时再次双击即可。

## 一天五次在场确认

每天有五个固定打卡点，每次有效期为约定时间前后各五分钟。

| 打卡点 | 有效窗口 | 含义 |
| --- | --- | --- |
| 09:00 | 08:55-09:05 | 开始打卡，确认已经就位 |
| 12:00 | 11:55-12:05 | 中午在场确认 |
| 15:00 | 14:55-15:05 | 下午在场确认 |
| 18:00 | 17:55-18:05 | 傍晚在场确认 |
| 21:00 | 20:55-21:05 | 结束打卡，并进入今日结算 |

中间的打卡只回答“我在”，不会自动开始、暂停或结束学习计时。超出窗口会如实记为错过，但不会打断已经开始的学习。

## 学习计时

- 双击小鹿后开始累计本段时间；再次双击结束。
- 一天可以记录任意多段，日累计时间会自动相加。
- 不记录“暂停了多久”，也不把学习时长和固定打卡强行绑定。
- 计时状态会同步显示在共学日记顶部。

## 学习启动陪伴

上午、下午和晚上，小鹿会分别留意一次学习是否顺利开始。检测到你正在使用电脑但五分钟仍未开始后，她只问一次：现在开始、十分钟后再提醒，或跳过这一段。每一段最多延后一次，选择跳过后不会追问。

选择开始或在空闲时双击小鹿，会进入一个五分钟的轻量启动仪式：YuQuiz 可用时替你打开网页，只陪你走到第一题。第一题完成后会给出一次特别反馈，随后回到普通状态，不干扰后续学习；若五分钟仍未开始，只再提醒最后一次。YuQuiz 不可用时会自动退回本地手动计时。

跳过不会成为失败记录，也不设置连续天数惩罚；启动监督只帮助当下开始，不再额外占用统计页面。

## 离线语音陪伴

固定打卡、学习启动、第一题完成、任务与悬赏成果、YuQuiz 暂停和晚间结算等重要节点可以播放小鹿的本地语音。所有语音均随应用安装，不调用在线 TTS，也不会上传文字或录音。统计页可以随时关闭语音并调整音量；点击日记上方的小鹿，她还会随机说一句话并匹配相应动作。新语音会自动打断旧语音，避免多条提醒重叠播放。

## 21 点今日结算

完成 21 点打卡后，可以在“今日”页填写：

- 今日做题数量；
- 一句最想留下的成果；
- 自己是否完成现实约定；
- 现实中的朋友是否完成约定。

学习时长和五次打卡由系统自动保存。对勾表示完成，叉表示未完成；再次点击当前选项可以撤销选择。

## YuQuiz 联动

共学日记会在本机检测 `http://127.0.0.1:8765`：开始使用 YuQuiz 时自动进入联动，停止活动或关闭网页后自动回到手动模式。统计页的开关仍可临时退出当前一段联动。

- 读取今日答题数量；答对/答错只作为短暂的桌宠动作反馈，不写入日记界面；
- 读取 YuQuiz 提供的实际学习时长；
- 由 YuQuiz 的活跃心跳判断是否正在学习；
- 连续 5 分钟没有网页操作后，YuQuiz 应暂停活跃学习计时；
- 自动暂停时，小鹿会用一枚气泡轻轻提醒；联动计时期间仍保持普通的鼠标跟随；
- YuQuiz 未启动或接口暂时不可用时，保留上一次数据并显示离线状态。

联动开启时，网页学习时间由 YuQuiz 负责记录；关闭开关后可以恢复共学日记的手动计时。两种来源的学习时长会在当天合并累计，不会因为切换开关而清空或覆盖。今日结算中的做题数量会跟随 YuQuiz 更新，提交结算时保存当时的数量。共学日记不会读取 YuQuiz 的 SQLite、题目、答案、API Key 或个人笔记，也不会向网络发送数据。

## 悬赏与每日任务

- 任务页顶部有两份每日悬赏：一份为自己赢书签，一份为她赢书签。
- 两份悬赏直接写在横向书签上，每天会自动刷新为未完成；清空文字可以恢复未填写状态。完成后双击书签，它会晃动并收起为一句完成寄语，右侧回车符号可以撤回。
- 未完成悬赏不会扣除书签，也不会留下失败标志。
- 在悬赏下方可以添加普通任务，点击方框切换完成状态。
- 任务超过三项、学习记录超过四天时会自动显示像素翻页条，不使用滚动条；较长的成果记录会折叠为两行，点击即可临时查看全文。
- 任务文字可直接在框内修改，按 Enter 或点击别处保存，按 Esc 撤销本次修改。
- 点亮“日”后，普通任务会升级为每日任务，每天自动生成一份新的未完成任务。
- 修改固定任务会同步更新后续日期；取消固定会保留今天的任务，但明天不再生成。
- 晚间仍有未完成任务时，小鹿会适度提醒，但任务结果不会替你判断现实约定是否完成。

## 书签规则

书签不是虚拟货币，也不会被消费。每一枚都来自主动完成的悬赏，或两个人共同守住约定的一天。

| 获得方式 | 生成内容 |
| --- | --- |
| 完成第一份每日悬赏 | 为自己赢得的书签 |
| 完成第二份每日悬赏 | 为她赢得的书签 |
| 晚间结算时两个人都完成约定 | 双人书签 |
| 悬赏或共同约定未完成 | 如实保留记录，不生成失败书签 |

“统计”页会累计学习时长、做题数量、按时打卡次数、完成任务与悬赏的数量，以及全部书签数量。系统不统计连续天数，偶尔中断不会变成需要找补的压力。

## 共学日记页面

- **今日**：今日计时、五次打卡、晚间结算。
- **任务**：完成两份每日悬赏，管理普通任务与每天刷新的每日任务。
- **记录**：过去每天的结算和一句成果。
- **统计**：累计数据、开机自启动和 YuQuiz 联动开关。
- **右上角书签入口**：查看为自己、为她赢得的书签与双人书签收藏。

所有页面均采用一屏布局；记录较多时使用分页，不在面板内部放置滚动条。

## 动作与状态

小鹿的动作不是随机装饰，而是她表达状态的方式：

- 平常等待或暂停：看向鼠标；
- 学习中：认真进行中的动作；
- 打卡提醒：等待回应；
- 开始计时或打卡成功：挥手回应；
- 结束一段学习：回顾动作；
- 两个人都完成约定：开心跳起；
- 错过打卡或未完成约定：短暂失落；
- 拖动：根据移动方向向左或向右跑。

21 点结算后会回到平常状态，并间隔一段时间短暂重现结算动作，不会一直跳动干扰桌面。

## 数据与隐私

小鹿没有账号系统、云同步、排行榜或社交上传。学习数据只保存在当前电脑：

```text
%APPDATA%\xiaolu-desktop-pet\xiaolu-study-state.json
```

卸载应用前如果希望保留记录，可以手动备份这个文件。应用的开机自启动可以在“统计”页或托盘菜单中关闭。

## 完整图文说明书

[下载 PDF 版说明书](docs/xiaolu-study-guide.pdf)

<details>
  <summary>在 README 中展开完整说明书长图</summary>
  <br />
  <img src="docs/images/xiaolu-study-guide-long.png" alt="共学日记说明书完整长图" />
</details>

## 本地开发

需要 Node.js 20 或更高版本，以及 pnpm。

```powershell
git clone https://github.com/UniqueYu8988/XiaoLu.git
cd XiaoLu
pnpm install
pnpm check
pnpm start
```

生成 Windows x64 MSI：

```powershell
pnpm package:msi
```

安装包输出到 `release/`。`package.json` 中的 MSI `upgradeCode` 已固定，后续发布新版本时不要修改它，只需要更新版本号并重新打包。

## 项目结构

```text
assets/                 图标、角色配置与动画图集
docs/                   PDF 说明书和 README 展示长图
scripts/                构建、图标、说明书与 MSI 辅助脚本
src/game.ts             学习记录、打卡、任务、书签和统计逻辑
src/main.ts             Electron 主进程、窗口、托盘与自启动
src/renderer/           桌面角色与共学日记界面
tests/                  核心状态逻辑测试
```

## 与 Codex Pet、OpenPets 的关系

- 角色动画素材最初按照 Codex v2 动画宠物素材规范整理，项目复用了现有的 `pet.json` 和 spritesheet。
- 本项目重新实现了独立 Electron 桌面程序、学习计时、固定打卡、每日任务、日记、书签、统计和本地存储，不要求安装 Codex。
- 桌面窗口与宠物架构的实现参考了 [OpenPets](https://github.com/alvinunreal/openpets) 的思路，相关说明见 [THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md)。

## 版本计划

当前为 `1.1.3`：修复普通点击被误判为拖动、导致学习驻守返程目标丢失的问题。现在只有鼠标移动超过阈值才会进入拖动；YuQuiz 返回 `study_state=closed` 后立即返程，且位置逻辑不受 `active_session` 影响。自由位置与学习驻守点继续分别保存。

后续功能会在实际使用一段时间后集中整理，再提升次版本号发布，避免为了增加功能而让学习助理本身变成负担。

## 许可证与素材授权

- 项目自行编写的源代码、测试、构建脚本和配置采用 [MIT License](LICENSE) 开源。
- `assets/` 与 `docs/` 中的小鹿角色、动画图集、角色图标及带有人物的说明书素材不属于 MIT，仍由项目所有者保留全部权利。
- 你可以自由使用和修改代码；如果没有获得额外素材授权，需要把小鹿角色素材替换成自己的内容。

完整边界和中英文说明见 [ASSET_LICENSE.md](ASSET_LICENSE.md)，第三方项目声明见 [THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md)。
